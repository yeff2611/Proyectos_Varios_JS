...
function groupBy(collection, it){
    try {
        let fnIt
        return collection.reduce((acumulador, valorActual)=>{  
            if(typeof(it) === 'string'){
                fnIt = eval(JSON.stringify(valorActual)+'.'+it)
            }else{
                fnIt = it(valorActual)
            }                  
            if(!acumulador[fnIt]) acumulador[fnIt]=[]
            acumulador[fnIt].push(valorActual)
            return acumulador
        },{}) 
    } catch (error) {
        return collection.reduce((acumulador, valActual)=>{
            let key = valActual[it]
            if(!acumulador[key]) acumulador[key] =[]
            acumulador[key].push(valActual)
            return acumulador
        },{})
    }         
}
...
/*function groupBy(collection, it){
    if(typeof(it) === 'string'){
        return collection.reduce((acumulador, valorActual)=>{
            long = eval(JSON.stringify(valorActual)+'.'+it)
            if(!acumulador[long]) acumulador[long]=[]
            acumulador[long].push(valorActual)
            return acumulador
        },{})
    }else{
        return collection.reduce((acumulador, valorActual)=>{
            let floorVal = it(valorActual)
            if(!acumulador[floorVal]) acumulador[floorVal]=[]
            acumulador[floorVal].push(valorActual)
            //acumulador[floorVal] = `${acumulador[floorVal]},${valorActual}`
            return acumulador
        },{})  
    }   
}*/